...
import discriminator as dis_module
import estimator_lib as est_lib
import eval_lib
...
def run_continuous_eval(hparams):
    ...
    for ckpt_str in evaluation.checkpoints_iterator(
        ...
        tf.compat.v1.logging.info(
            'Finished evaluating checkpoint: %s' %
            ckpt_str)
...
def run_train_and_eval(hparams):
    ...
    while cur_step < max_step:
        if hparams.tpu_params.use_tpu_estimator:
            tf.compat.v1.logging.info(
                'About to write sample images at step: %i' %
                cur_step)
            eval_lib.predict_and_write_images(
                estimator,
                train_eval_input_fn,
                hparams.model_dir,
                'step_%i' %
        ...
        step_to_stop_at = int(step_to_stop_at)
        tf.compat.v1.logging.info(
            'About to train to step: %i' %
            step_to_stop_at)
        start = time.time()
...
def run_eval(hparams):
    """Evaluate the selected checkpoint by N time."""
    estimator = make_estimator(hparams)
    
    if hparams.load_dir is not None:
        tf.compat.v1.logging.info(
            'Load checkpoint from %s' %
            hparams.load_dir)

    start_time = time.time()
    for cur_eval in range(hparams.total_eval_times):
        # Sample images from generator.
        if hparams.tpu_params.use_tpu_estimator:
            tf.compat.v1.logging.info(
                'About to write sample images at evaluation: %i' %
                cur_eval)
            eval_lib.predict_and_write_images(
                estimator,
                train_eval_input_fn,
        ...
        # Run evaluation.
        tf.compat.v1.logging.info('Evaluating at %i' % cur_eval)
        estimator.evaluate(
            train_eval_input_fn, steps=hparams.num_eval_steps, name='eval')
        tf.compat.v1.logging.info('Finished evaluaion')
...
def _get_discriminator(hparams):
    ...
    def discriminator(images_and_lbls, unused_conditioning, mode):
        ...
        if hparams.debug_params.fake_nets:
            ...
            logits = tf.zeros([tf.shape(input=images)[0],
                               20]) * tf.compat.v1.get_variable('dummy_d',
