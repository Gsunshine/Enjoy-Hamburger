...
flags.DEFINE_integer('G_steps', 1,
                    'Steps for G\'s update.')
flags.DEFINE_integer('D_steps', 1,
                    'Steps for D\'s update.')
...
def get_tpu_run_config_from_hparams(hparams):
    ...
    return tf.compat.v1.estimator.tpu.RunConfig(
        ...
        keep_checkpoint_max=hparams.max_ckpts,
...
def get_tpu_estimator(generator, discriminator, hparams, config):
    return tfgan.estimator.TPUGANEstimator(
        ...
        gan_train_steps=tfgan_tuples.GANTrainSteps(flags.FLAGS.G_steps, flags.FLAGS.D_steps),
        ...
        warm_start_from=os.path.join(hparams.model_dir, hparams.load_dir) if hparams.load_dir else None)
