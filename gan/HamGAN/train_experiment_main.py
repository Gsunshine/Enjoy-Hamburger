...
flags.DEFINE_string('load_dir', None,
                    'Optional path to load checkpoint from model_dir for evaluation. '
                    'If `None`, use the checkpoint record in model_dir by default.')
...
# ML Infra.
flags.DEFINE_enum(
    'mode', None, ['train', 'continuous_eval', 'train_and_eval', 'eval'],
    ...
    '`eval` evaluates a given checkpoint from load+dir. '
...
flags.DEFINE_integer('num_eval_steps', 49, 'The number of evaluation steps.')
...
flags.DEFINE_integer('total_eval_times', 5,
                     'Total times for evaluating a checkpoint')
flags.DEFINE_integer('max_ckpts', 5,
                     'Max checkpoints to keep during training.')
...
def main(_):
    ...
    hparams = train_experiment.HParams(
        ...
        total_eval_times=FLAGS.total_eval_times,
        max_ckpts=FLAGS.max_ckpts,
    ...
    if FLAGS.mode == 'train':
        ...
        train_experiment.run_eval(hparams)
    else:
